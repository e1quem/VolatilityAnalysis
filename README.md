...
#### 1. [utils.py](globalAnalysis/utils.py)
...
- **Sufficient price change**: to measure volatility, we need price movement. Hence, we filter out markets whose price hasn't moved enough in the last month, week, or day. The goal isn't to focus on large price movements, but to avoid static markets, for example those for which the solution is already public, leading to a fixed price until the last trading day.
...
#### 2. [returnsDistribution.py](globalAnalysis/returnsDistribution.py)
...
Thus, using a normal distribution for our GARCH analysis wouldn't be accurate. From now on, we'll use the *Generalized Error Distribution* (GED) of the arch library to fit our models, to better capture sharp peak and fat tails.
...
#### 4. [getData.py](globalAnalysis/getData.py)
Used to download .csv price history of eligible markets in data/Politics and data/Sports. The maximum granularity is 10m intervals for 30 days of historical data.


## GARCH Models

#### 1. [GARCHbacktest.py](models/GARCH/GARCHbacktest.py)
This file uses GARCH models to backtest volatility forecasts on individual markets (requires an individual market slug to run). The model is fitted on an initial amount of training points. The volatility forecast it outputs is then used to define a 95% confidence price interval forecast for the next aggregated time period. This 95% interval doesn't use a normal distribution, but uses empirical quantiles of standardized residuals. Then, 10m observed prices are used to measure hits and misses (price is inside/outside the interval).

Instead of fitting only one model at each step, we take a more adaptive approach. For each aggregated period, we fit 6 different models: ARCH(1), ARCH(2), GARCH(1,1), GARCH(2,1), TARCH(1,1,1), TARCH(2,2,1). We then compare their BICs and use the model with the lowest one for the forecast.
...
#### 2. [GARCHbacktests.py](models/GARCH/GARCHbacktests.py)
...
#### 3. [RogersSatchell.py](models/GARCH/RogersSatchell.py)

GARCH models can use exogenous variables in their forecasts. For volatility models, volume can be used. However, historical volume is not obtainable via Polymarket API. Obtaining it would require a constant Websocket connection hoarding data for a list of markets for weeks. *This could become a future project in order to bypass the 10m maximum fidelity and 30 days limit on price history, as well as the lack of other variables such as volume, spread and order book dynamics.*

Still, we can use additional information by transforming our aggregated price data into price candles, since they carry more information than simple closing prices.

In this sense, we use the **Rogers & Satchell (1991)** formula as an exogenous variable for GARCH(1,1) and TARCH(1,1,1) models:
...
Where $O_i$ is the opening price, $C_i$ is the closing price, $H_i$ is the highest price and $L_i$ is the lowest price.
...
#### 1. [ornsteinUhlenbeck.py](models/ornsteinUhlenbeck/ornsteinUhlenbeck.py)
...
We estimate these parameters by fitting an AR(1) process on rolling realized volatility. The 20-period rolling window was arbitrarily chosen. This OU model can be run on multiple markets, and yields relatively high accuracy.
...
Since our data has a relatively high granularity (10m), using GARCH models may be less suitable: these models were designed to perform on daily prices for extended periods of time, not on intraday price movements.

To solve this issue, we use the HAR-RV model. Its traditional financial version takes into account different time horizons for short-term volatility forecasts: daily, weekly and monthly volatility (1:5:22 ratio). Its simple autoregressive structure has a long memory with faster computation speed than the slow GARCH BIC-comparison method.

#### 1. [ratioTesting.py](models/HAR-RV/ratioTesting.py)

This testing file compares different ratios on individual markets (requires individual market slug). The standard ratio of the HAR-RV model is 1:5:22. For constantly running markets, this ratio becomes 1:7:30. Users can add testing ratios in the file. *Note: 1 unit corresponds to a 10m time period.*

Rankings produced by this file were used to define ratios used for HAR-RV backtests.

#### 2. [HARbacktest.py](models/HAR-RV/HARbacktest.py)

This file follows the same structure as ```GARCHbacktest.py``` but uses HAR-RV instead of GARCH models to forecast volatility. HAR-RV ranges are adaptive according to the amount of available historical data.
...
This file shares the same structure as ```HARbacktest.py```. It tests multiple markets with market type and volume filtering. It provides residuals analysis and Durbin-Watson calculation for each market in order to account for autocorrelation of residuals.
...
- Log-returns on Sports and Politics prediction markets exhibit **extremely high kurtosis**, with a sharp peak and fat-tailed distribution.
...
- **Model performance** across high-volume markets:
    * *GARCH BIC-comparison method* obtained a **93.8% average accuracy** (23,332 misses out of 389,435 data points across 122 markets). **Up and down misses are balanced**: respectively 49.93% and 50.07%. The program couldn't fit the models for 11 markets out of 133. The most used model was **ARCH(1)**. This indicates that more complex models such as GARCH and TARCH are not necessarily better for the price behavior of prediction markets.
    *  *HAR-RV method* obtained a **95.81% average accuracy** (20,297 misses out of 502,162 data points for 133 markets). Similar balance for up and down misses: respectively 50.7% and 49.3%.
    * *Ornstein-Uhlenbeck process* obtained a **93.23% average accuracy** over 115 markets.


#### 2. Limitations

- Due to the low granularity of data, it is impossible to use HFT intraday models that would run on minute intervals, or less.
...
- Implementing up and down misses as trading signals for a theoretical trading project. Detecting these abnormal price movements could spot sudden arrival of new information, which could in turn lead to price overreaction amplified by volatility traders and followed by mean-reversing periods.
