...
- **High Volume**: we used high-volume markets (1M+ volume) for these analysis. This setting can be modified by user input for each program.
...
Log-returns on high-volume prediction markets are super-leptokurtik: they have a highly positive Kurtosis. Thus, using a normal distribution for our GARCH analysis wouldn't be accurate. From now on, we'll use the *Generalized Error Distribution* (GED) of the garch library.

#### 3. ```skewSmile.py```
The statistical distribution of log-returns already gives an approximation of the skewness of the data. In order to observe its distribution and eventual smile or skew, we can plot observed volatility according to price using a simple ARCH(1) model. We'll use more complex models later on.
![Skew](assets/VolatilitySkew.png)
This graph does not necessarily indicates volatility skewness. It simply reflects higher relative changes for small price magnitudes, and lower relative changes for higher prices, with a plateau from 30c to 70c.
