...
#### 1. [utils.py](./utils.py)
To conduct volatility analysis, we focus on two types of prediction markets: Sports and Politics. These are the most common and most popular and liquid markets on Polymarket.
Among these categories, we select specific markets: 
- **High Volume**: we used high-volume markets (1M+ volume) for this analysis. *This setting can be modified via user input*.
...
GARCH models can use exogenous variables in their forecasts. For volatility models, volume can be such additional variable. However, historical volume data is not obtainable via Polymarket API. Obtaining this information would require constant Websocket connection for a list of markets, and data hoarding for weeks. This could become a future project in order to bypass the 10m maximumum fidelity and 30 days limit on price history, as well as the lack of other variables such as volume, spread and order book dynamics.
...
Since our data has a relatively high granularity (10m), using GARCH models is sub-optimal: these traditional models were designed to perform on daily prices for extended periods of time, not on intraday price movements.

To solve this issue, we use the HAR-RV model. Its traditional financial version takes into account different time horizons for short-term volatility forecasts: daily, weekly and monthly volatility (1:5:22 ratio). Its simple autoregressive structure takes into account different time periods and has a large memory with faster computation speed than the slow GARCH BIC-comparison method.
